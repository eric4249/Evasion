...
int main() {
  ...
  // 此處使用原始 shellcode 作為示範
  // 若要整合手動 PE 映射，可在此處替換為 PE 檔案的原始位元組緩衝區
  ...
  // 此技術對應手動 PE 映射課程中的規避策略：
  // - 避免單次大型 VirtualAlloc（>64KB），降低 EDR 對「分配-寫入-保護變更-執行」序列的偵測
  // - 將酬載拆散成小塊，分散在目標進程的隨機記憶體位置，破壞 VAD 樹分析中對連續私有可執行記憶體的掃描
  // - 最終重組時才建立可執行區域，縮短可被記憶體掃描器捕捉的時間窗口
  ...
  // 7. 執行
  // 設定最終區域為可執行權限，模擬手動映射中「逐區段設定權限」的概念
  DWORD oldProt;
